(function registerColumnModel(namespace) {
  const { createId, nowIso } = namespace.shared;

  const DEFAULT_COLUMN_DEFINITIONS = Object.freeze([
  ...
  function createDefaultColumns(projectId, options = {}) {
    if (!projectId) {
  ...
  Object.assign(namespace.domain, { DEFAULT_COLUMN_DEFINITIONS, createDefaultColumns });
